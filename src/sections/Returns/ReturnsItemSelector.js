...
export function ReturnsItemSelector({ selectedOrder, selectedItems, handleSelectedItem, handleReasonChange, handleCommentChange, handleQuantityChange, handleOpenTicketDialog }) {
    ...
    if (!selectedOrder || !selectedOrder.returnableFulfillment || selectedOrder.returnableFulfillment.length === 0) {
        return (
            ...
            <Grid container spacing={2}>
                ...
                {selectedOrder &&
                    ...
                    <Grid item xs={12} md={8}>
                        <Stack>
                            {selectedOrder.returnableFulfillment.returnableFulfillmentLineItems.edges.map((item) => (
                                <Paper
                                    key={item.node.fulfillmentLineItem.id}
                                    sx={{
                                        padding: 2,
                                        marginBottom: 2,
                                        border: handleIsSelectedItem(item.node.fulfillmentLineItem.id) ? '2px solid #000' : 'none',
                                    }}
                                >
                                    <Grid container>
                                        <Grid item xs={2}>
                                        ...
                                        <Grid item xs={10}>
                                            ...
                                            >
                                                <Stack>
                                                    ...
                                                    <Typography >
                                                        {item.node.fulfillmentLineItem.lineItem.name}
                                                    </Typography>
                                                </Stack>
                                                <Checkbox
                                            ...
                                            >
                                                ...
                                                <Stack>
                                                    ...
                                                    {item.node.fulfillmentLineItem.discountedTotalSet.presentmentMoney.amount} {item.node.fulfillmentLineItem.discountedTotalSet.presentmentMoney.currencyCode}
                                                </Stack>
                                            ...
                                            <Collapse in={handleIsSelectedItem(item.node.fulfillmentLineItem.id)} timeout="auto" unmountOnExit>
                                                <Stack spacing={2} sx={{ marginTop: 2 }}>
                                                    <Stack direction="row" spacing={2}>
                                                        ...
                                                        <Stack direction="row" spacing={2}>
                                                            <TextField
                                                                ...
                                                                type="number"
                                                                value={item.node.fulfillmentLineItem.returnQuantity || item.node.fulfillmentLineItem.lineItem.refundableQuantity}
                                                                onChange={
                                                                    (event) =>
                                                    ...
                                                    <TextField
                                                        ...
                                                        multiline
                                                        rows={4}
                                                        onChange={(event) => handleCommentChange(item.node.fulfillmentLineItem.id, event.target.value)}
                    ...
                    <Grid item xs={12} md={8}>
                        <Typography>
                            Error getting returnable line items, please try again.
                        </Typography>
                    </Grid>
                )}
            </Grid>
