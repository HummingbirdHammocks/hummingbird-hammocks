...
export function ReturnsStepper() {
  ...
  const saveReturnReason = (id, value) => {
    ...
    if (lineItems && lineItems.length !== 0) {
      const newItem = lineItems.find((item) => item.id === id);
      if (newItem) {
    ...
    await axios
      ...
      .then((res) => {
        ...
        if (res.data && res.data.data.returnRequest && res.data.data.returnRequest.return) {
          ...
          res.data &&
          res.data.data.returnRequest &&
          res.data.data.returnRequest.userErrors
        ) {
          console.log(res);
    ...
    <Box sx={{ width: '100%', marginBottom: 5 }}>
      ...
      {activeStep === steps.length ? (
        <React.Fragment>
          ...
          <Typography sx={{ mt: 2, mb: 1 }}>
            If you would like to start a new request, please click the "reset" button below or
            refresh the page.
          </Typography>
