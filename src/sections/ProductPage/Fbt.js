...
export const Fbt = ({ currentVariant, product, fbtData }) => {
  ...
  useEffect(() => {
    ...
    items.forEach(
      (item, index, arr) => {
        getProductById(item).then((result) => {
          ...
          const getAvailable = result.variants.filter((i) => i.available === true);
    ...
    function setAllData() {
      ...
      const firstVariant = product.variants.filter(({ id }) => id === currentVariant.id);
  ...
  const handleSelectChange = (itemIndex, value) => {
    ...
    const findVariant = data[itemIndex].variants.filter(({ id }) => id === value);
              ...
              <>
                {selectedVariant.map((item, index) => (
                  <Box key={item.id} display="flex">
                    <Checkbox onChange={() => handleCheckChange(index)} checked={item.selected} />
                    <Box
                      sx={{
