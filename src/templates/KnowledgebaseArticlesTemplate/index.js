...
const KnowledgebaseArticles = ({ data: { knowledgebaseArticles, recentKnowledgebaseArticles }, pageContext: { next, prev } }) => {
  ...
  return (
    <Layout>
      <ArticlesHeader title={title} backpath={"/knowledgebase/articles"} date={fShopify(published_at)} />
      <MainWrapper>
        <Stack
        ...
        >
          <Typography variant="collectionName">
            ...
            <Link to={`/knowledgebase/articles/`}>ARTICLES</Link> /{" "}
            {title}
          </Typography>
            ...
            {prev && prev.handle && (
              <Tooltip title={prev.title}>
                <Button
                  size="small"
                  component={Link}
                  to={`/knowledgebase/articles/${prev.handle}`}
                >
                  Prev
                </Button>
              </Tooltip>
            ...
            {next && next.handle && (
              <Tooltip title={next.title}>
                <Button
                  size="small"
                  component={Link}
                  to={`/knowledgebase/articles/${next.handle}`}
                >
                  Next
                </Button>
              </Tooltip>
